...
export const FirstTimeAppLoaded = () => ({
  ...
  export const RegisterSuccess = (user, profile, token) => {
    localStorage.setItem("token", token);
    axios.defaults.headers["Authorization"] = `Token ${token}`;
  ...
  export const Register = (data) => {
    return (dispatch) => {
      ...
      axios
        .post("/eduction_apps/register/", data, {
          headers: headers
        })
        .then((res) => {
          dispatch(RegisterSuccess(res.data.userDetails.user_id, res.data.userDetails.firstname, res.data.token));
          console.log('axios data in login',res);
        })
        .catch((err) => {
          dispatch(LoginFailed(err.response));
